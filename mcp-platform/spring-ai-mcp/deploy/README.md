...
platform-server에서 발급한 API 키를 `Authorization: Bearer <api-key>` 헤더로 전달합니다.  
spring-ai-mcp는 `PLATFORM_URL`에 설정된 platform-server의 `/api/v1/api-keys/validate` 엔드포인트를 호출해 키를 검증하며, 결과는 5분간 인메모리 캐시에 보관합니다.

**API 키 발급:**  
platform-server에 로그인한 뒤 `/api/v1/api-keys` (POST)를 호출하거나 admin-front UI에서 생성합니다.  
키는 `sk-` 접두사의 Base64 URL-safe 문자열입니다.

**환경변수:**

| 변수 | 설명 |
|------|------|
| `PLATFORM_URL` | platform-server 주소 (예: `http://platform-server:8080`) |
...
| `PLATFORM_URL` | `http://platform-server:8080` | platform-server 주소 |
