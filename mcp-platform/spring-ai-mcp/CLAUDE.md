...
# platform-server URL을 명시적으로 지정할 경우 (기본값: http://localhost:8080)
PLATFORM_URL=http://localhost:8080 SPRING_PROFILES_ACTIVE=local ./gradlew bootRun
...
**로컬 실행 전제 조건:** `local` 프로파일은 `localhost:5432/sample_db`에 PostgreSQL이 필요하다. `deploy/` compose로 DB만 먼저 기동하거나 직접 띄운 뒤 `deploy/initdb/init.sh`로 테이블을 초기화한다. `application-local.yml`에 `app.auth.platform-url: http://localhost:8080`이 설정되어 있으므로 platform-server가 먼저 실행 중이어야 API 키 검증이 동작한다.
...
클라이언트가 `Authorization: Bearer <api-key>` 헤더를 보내면, `ApiKeyAuthFilter`가 platform-server의 `/api/v1/api-keys/validate` 엔드포인트를 호출해 키를 검증한다. 유효한 키이면 SecurityContext에 username과 role을 설정한다. 검증 결과는 5분 인메모리 캐시에 보관한다.
...
ForwardedHeaderFilter (HIGHEST_PRECEDENCE)
  ...
  → ApiKeyAuthFilter: Bearer <api-key> → platform-server 검증 → SecurityContext
```

#### 인증 흐름

1. 관리자가 admin-front(또는 platform-server API)로 API 키 생성
2. 생성된 `sk-...` 키를 MCP 클라이언트에 전달
3. 클라이언트: `Authorization: Bearer sk-...` 헤더로 MCP 호출
4. spring-ai-mcp: platform-server에 키 검증 요청 (5분 캐시)
5. 검증 성공 시 username·role → SecurityContext

#### 환경변수

| 변수 | 필수 | 설명 |
|---|---|---|
| `PLATFORM_URL` | 필수 | platform-server 주소. 기본값: `http://localhost:8080` |

`local` 프로파일은 `application-local.yml`의 `http://localhost:8080`을 사용하므로 환경변수 불필요.

#### 구현 파일

| 파일 | 역할 |
|------|------|
| `global/security/apikey/ApiKeyProperties.java` | `app.auth.platform-url` 바인딩 |
| `global/security/apikey/ApiKeyAuthFilter.java` | Bearer 토큰 추출 → platform-server 검증 → SecurityContext |
