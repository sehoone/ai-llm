...
class UserRepositoryMixin:
    ...
    async def get_user_by_keycloak_id(self, keycloak_id: str) -> Optional[User]:
        """Retrieve a user by Keycloak subject UUID.

        Args:
            keycloak_id: The Keycloak user UUID (JWT sub claim).
    ...
    async def create_keycloak_user(
        ...
        """Create a user provisioned from a Keycloak JWT (no local password).

        Args:
            keycloak_id: Keycloak subject UUID.
            email: Email from the Keycloak token.
            username: preferred_username from the Keycloak token.
            role: Mapped UserRole from Keycloak realm roles.

        Returns:
            User: The newly created user record.
        """
        from src.user.models.user_model import UserRole as _UserRole
        ...
        def _sync():
            with Session(self.engine) as db:
                ...
                db.commit()
                db.refresh(user)
                logger.info("keycloak_user_provisioned", keycloak_id=keycloak_id, email=email)
                return user
        return await asyncio.to_thread(_sync)

    async def update_user_keycloak_id(self, user_id: int, keycloak_id: str) -> None:
        """Link an existing user to a Keycloak subject UUID.

        Args:
            user_id: The local user's integer ID.
            keycloak_id: The Keycloak UUID to associate.
        """
        def _sync():
            with Session(self.engine) as db:
                user = db.get(User, user_id)
                if user:
                    ...
                    db.add(user)
                    db.commit()
                    logger.info("user_keycloak_id_linked", user_id=user_id, keycloak_id=keycloak_id)
