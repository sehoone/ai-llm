...
<template>
  <div class="flex-grow-1 d-flex align-center justify-space-between">
    <v-btn title="Tools" :icon="getToolIcon()" density="compact" size="default" class="mr-3" id="tools_btn"> </v-btn>
    <v-menu activator="#tools_btn" open-on-hover>
      <v-list density="compact">
        ...
        <v-list-item prepend-icon="article" @click="docDialogCtl.dialog = true"> Documents </v-list-item>
      </v-list>
    </v-menu>
    <v-textarea
      ref="textArea"
    ...
    ></v-textarea>
    <v-btn :disabled="loading" icon="send" title="Send" class="ml-3" @click="clickSendBtn"></v-btn>
  </div>
  <DocumentsManage :send-message="sendMessage" :control="docDialogCtl" ref="documentMan" />
</template>
